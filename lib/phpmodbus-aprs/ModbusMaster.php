 ...
 *   
 * @author Jan Krakora
 * @author with additions by James Jarvis / APRS World, LLC
 * @copyright  Copyright (c) 2004, 2013 Jan Krakora
 * @package phpmodbus-aprs  
 *
 */
class ModbusMaster {
	private $sock;
	 ...
	 *     
	 * @param String $host An IP address of a Modbus TCP device. E.g. "192.168.1.1" with option :port 
	...
	function ModbusMaster($host, $protocol){
		...
		/* check to see if we have a ':port' suffix on hostname */
		$cpos=strpos($host,':');
		if ( false === $cpos ) {
			...
			/* port specified */
			$this->host = substr($host,0,$cpos);
			$this->port = substr($host,$cpos+1);
		}

//		printf("##### host: %s port: %s protocol: %s #####\n",$this->host,$this->port,$this->socket_protocol);
	}
	...
	private function rec(){
		...
		$rec = "";
		$lastAccess = time();
		while ( socket_select($readsocks, $writesocks, $exceptsocks, $tv_sec, $tv_usec) !== FALSE) {
	...
	private function responseCode($packet){    
		...
		if((ord($packet[7]) & 0x80) > 0) {
			...
			// mbusd seems introduce 0x128=>MB_EX_CRC and 0x129=>MB_EX_TIMEOUT */
			$failures = array(
					...
					128  => "MBUSD CRC EXCEPTION",
					129  => "MBUSD TIMEOUT EXCEPTION");
			// get failure string
	...
	function writeMultipleRegister($unitId, $reference, $data, $dataTypes){
		$this->status .= "writeMultipleRegister: START (unitId=" . $unitId . ")\n";
		// connect
		$this->connect();
		// send FC16    
	...
	private function writeMultipleRegisterPacketBuilder($unitId, $reference, $data, $dataTypes){
		...
		$buffer3 = '';
		//    $buffer3 .= iecType::iecINT(rand(0,65000));   // transaction ID    
		$buffer3 .= iecType::iecINT(257);   // transaction ID JJJ
		$buffer3 .= iecType::iecINT(0);               // protocol ID    
		$buffer3 .= iecType::iecINT($dataLen + 1);    // lenght    
		$buffer3 .= iecType::iecBYTE($unitId);        //unit ID    
	...
	private function printPacket($packet){
		...
		$str .= "Packet: "; 
		$cnt  = "Index:  ";
		$len  = "Length: " . strlen($packet) . "\n";
		for($i=0;$i<strlen($packet);$i++){
			//$str .= $this->byte2hex(ord($packet[$i])) . ' ';
			$str .= sprintf("0x%02x ",ord($packet[$i]));
			$cnt .= sprintf("%-2d   ",$i);
		}
		$str .= "\n";
		$cnt .= "\n";
